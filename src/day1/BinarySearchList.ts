export default function bs_list(haystack: number[], needle: number): boolean {
    ...
    do{
        const m = Math.floor(lo + (hi - lo) /2) // math to get middle index
        const v = haystack[m] // check middle index
        if (v === needle) {
            return true
        } else if (v > needle) {
    ...
    return false // made it though the loop without finding anything
}